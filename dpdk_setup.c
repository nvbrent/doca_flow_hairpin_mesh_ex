...
bool hairpin_mesh_enabled =
#ifdef DOCA_ENABLE_HAIRPIN_MESH
    true;
#else
    false;
#endif
...
bind_hairpin_queues(uint16_t port_id)
{
	...
	for (peer_port = 0; peer_port < peer_ports_len; peer_port++) {
        DOCA_LOG_INFO("rte_eth_hairpin_bind(%d,%d)", port_id, peer_ports[peer_port]);
	...
	for (peer_port = 0; peer_port < peer_ports_len; peer_port++) {
        DOCA_LOG_INFO("rte_eth_hairpin_bind(%d,%d)", peer_ports[peer_port], port_id);
...
port_init(struct rte_mempool *mbuf_pool, uint8_t port, struct application_dpdk_config *app_config)
{
    ...
    if (hairpin_mesh_enabled) {
        if (nb_hairpin_queues) {
            for (queue_index = 0; queue_index < nb_hairpin_queues; queue_index++) {
                uint16_t peer_port_id = queue_index;

                if (!rte_eth_dev_is_valid_port(peer_port_id)) {
                    DOCA_LOG_WARN("Port %d: hairpin index %d: skipping invalid peer-port %d", 
                        port, rss_queue_list[0], peer_port_id);
                    continue;
                }

                uint16_t lcl_hairpin_q = app_config->port_config.nb_queues + peer_port_id;
                uint16_t rmt_hairpin_q = app_config->port_config.nb_queues + port;
                ...
                struct rte_eth_hairpin_conf hairpin_conf = {
                    ...
                    .manual_bind = !!manual,
                    .tx_explicit = !!tx_exp,
                    .peers[0] = {peer_port_id, rmt_hairpin_q},
                };

                // TX
                DOCA_LOG_INFO("rte_eth_tx_hairpin_queue_setup(%d, %d) -> {%d, %d}",
                    port, lcl_hairpin_q, peer_port_id, rmt_hairpin_q);
                result = rte_eth_tx_hairpin_queue_setup(port, lcl_hairpin_q, nb_tx_rx_desc, &hairpin_conf);
                if (result < 0) {
                    DOCA_LOG_ERR("Failed to setup hairpin queues (%d)", result);
                    return DOCA_ERROR_DRIVER;
                }
                ...
                // RX
                DOCA_LOG_INFO("rte_eth_rx_hairpin_queue_setup(%d, %d) -> {%d, %d}",
                    port, lcl_hairpin_q, peer_port_id, rmt_hairpin_q);
                result = rte_eth_rx_hairpin_queue_setup(port, lcl_hairpin_q, nb_tx_rx_desc, &hairpin_conf);
                if (result < 0) {
                    DOCA_LOG_ERR("Failed to setup hairpin queues (%d)", result);
                    return DOCA_ERROR_DRIVER;
                }
            }
            DOCA_LOG_INFO("Port %d: Configured hairpin mesh queues [%d..%d] to ports [%d..%d]",
                port, app_config->port_config.nb_queues, 
                app_config->port_config.nb_queues + nb_hairpin_queues - 1,
                0, nb_hairpin_queues - 1);
        } else {
            DOCA_LOG_INFO("Port %d: no hairpin queues configured", port);
        }
    }
    else /* revert to 'dpdk_utils.c' implementation */
    {
